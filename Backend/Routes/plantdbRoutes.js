...
router.post('/PlantRecommend', (req, res) => {
    const randomTime = Math.floor(Math.random() * 60000);
    const query = `SELECT Pname, Image FROM plantdb ORDER BY RAND(${randomTime}) LIMIT 10`;
...
router.post('/test', (req, res) => {
    ...
    const query = 'SELECT * FROM plantdb WHERE mbti LIKE ?';
...
router.post('/test/Pname', (req, res) => {
    const { Pname } = req.body;
    const query = 'SELECT * FROM plantdb WHERE Pname = ?';
